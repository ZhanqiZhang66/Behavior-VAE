    ...
    for motif in range(n_cluster):
        ...
        for i, j in itertools.combinations(range(len(df_epoch_motif_HC)), 2):
            ...
            if len(latent_vector_HC) == 0 or len(latent_vector_BD) == 0:
                ...
                BD_HC_distance = wasserstein_distance_nd(latent_vector_BD, latent_vector_HC)
...
distances_BD_HC_df = pd.DataFrame(distances_BD_HC)
distances_BD_HC_df.to_csv(f'{project_path}/data/Wasserstein_distances_BD_HC.csv', index=False)
