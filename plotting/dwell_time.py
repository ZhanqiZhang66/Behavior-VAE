        ...
        if n_cluster == 10:
            control_label = np.load(
                r'{}\Behavior_VAE_data\{}\results\{}\VAME\kmeans-{}\DLC_{}_km_label_{}.npy'.format(onedrive_path,
                                                                                                   ...
                                                                                                   n_cluster, n_cluster,
                                                                                                   v))
            control_cluster_center = np.load(
                r'{}\Behavior_VAE_data\{}\results\{}\VAME\kmeans-{}\DLC_{}_cluster_center_{}.npy'.format(onedrive_path,
                                                                                                         project_name,
                                                                                                         v, n_cluster,
                                                                                                         n_cluster, v))
...
for i in range(n_cluster):
    ...
    print("          Pearson corr YMARS-BD: rho: {:.2f}, p-val: {:.2f}".format(corr_YMRS_score_BD[0][0],
                                                                               corr_YMRS_score_BD[1]))
    print("          Pearson corr HAM_D-BD: rho: {:.2f}, p-val: {:.2f}".format(corr_HAM_D_score_BD[0][0],
                                                                               corr_HAM_D_score_BD[1]))
    ...
    print("          Pearson corr YMARS-HC: rho: {:.2f}, p-val: {:.2f}".format(corr_YMRS_score_ctl[0][0],
                                                                               corr_YMRS_score_ctl[1]))
    print("          Pearson corr HAM_D-HC: rho: {:.2f}, p-val: {:.2f}".format(corr_HAM_D_score_ctl[0][0],
                                                                               corr_HAM_D_score_ctl[1]))
    # only correlate with BD list
...
for k in range(3):
    ...
    ds = pd.DataFrame(np.concatenate((
        np.concatenate((motif_usage_to_plot_[0, :, :].T.flatten(), motif_usage_to_plot_[1, :, :].T.flatten()),
                       0).reshape(-1, 1),
...
for i in range(n_cluster):
    states.append([i] * n_subject_in_population)
states = np.asarray(states).flatten()
HC_idx = np.zeros(n_subject_in_population * len(bahavior_names))
BD_idx = np.ones(n_subject_in_population * len(bahavior_names))
motif_usage_cat_score = np.array(motif_usage_cat_score)
ds = pd.DataFrame(np.concatenate((
    np.concatenate((motif_usage_cat_score[0, :, :10].T.flatten(), motif_usage_cat_score[1, :, :10].T.flatten()),
                   0).reshape(
        -1, 1),
    np.concatenate((HC_idx, BD_idx), 0).reshape(-1, 1),
    np.concatenate((states, states), 0).reshape(-1, 1)), 1),
...
labels = [dict_name[float(text.get_text())] for text in ax.legend_.texts]
# sns.swarmplot(y="motif frequency", x="state", hue='is_BD',data=ds,dodge=True,size=2)
x = np.arange(len(bahavior_names))
ax.legend(handles, labels)
ax.set_xticks(x)
ax.set_xticklabels(["sit", "sit_obj", "stand", "stand-obj", "walk", "walk_obj", "lie", "lie_obj", "interact", "wear"])
fig.show()
pwd = r'{}\Behavior_VAE_data\{}\figure\dwell-time'.format(onedrive_path, project_name)
Path(pwd).mkdir(parents=True, exist_ok=True)
...
# Plot Box plot Fig. 2e right
bahavior_names = ["sit", "sit_obj", "stand", "stand-obj", "walk", "walk_obj", "lie", "lie_obj", "interact", "wear"]
merged_behavior_names = ["sit", "stand", "walk", "lie", "interact", "wear"]
merged_states = []
for i in range(6):
...
ds = pd.DataFrame(np.concatenate((
    np.concatenate((motif_usage_to_plot_[0, :, :].T.flatten(), motif_usage_to_plot_[1, :, :].T.flatten()), 0).reshape(
        -1, 1),
    np.concatenate((HC_idx, BD_idx), 0).reshape(-1, 1),
...
for i, motif_behavior in enumerate(bahavior_names):
    HC_score = motif_usage_to_plot_[0, :, i].reshape(-1, 1)
    BD_score = motif_usage_to_plot_[1, :, i].reshape(-1, 1)
    ...
    # print("motif  {}, permutation_test: {:.2f}, p-val: {:.3f}".format(i,res.statistic, res.pvalue))
    corr_HAM_D_score_score = scipy.stats.pearsonr(motif_usage_to_plot_[0, :, i], HAM_D_score[:n_subject_in_population])
    corr_YMRS_score_score = scipy.stats.pearsonr(motif_usage_to_plot_[0, :, i], YMRS_score[:n_subject_in_population])
    ...
    # only correlate with BD list
    corr_HAM_D_score_BD_score = scipy.stats.pearsonr(motif_usage_to_plot_[1, :, i],
                                                     HAM_D_score[n_subject_in_population:])
    corr_YMRS_score_BD_score = scipy.stats.pearsonr(motif_usage_to_plot_[1, :, i],
                                                    YMRS_score[n_subject_in_population:])
...
HC_idx = np.zeros(n_subject_in_population * n_cluster)
BD_idx = np.ones(n_subject_in_population * n_cluster)
motif_usage_cat = np.array(motif_usage_cat)
ds = pd.DataFrame(np.concatenate((
...
fig, ax = plt.subplots(1, 1, figsize=(w, 4))
violin = sns.boxplot(y="motif frequency", x='state', hue='is_BD',
                     data=ds, palette=sns.color_palette("tab10"), orient='v')
handles = violin.legend_.legendHandles
dict_name = {0.0: 'HC', 1.0: 'BD'}
labels = [dict_name[float(text.get_text())] for text in ax.legend_.texts]
# sns.swarmplot(y="motif frequency", x="state", hue='is_BD', data=ds, dodge=True, size=2)
...
for epoch in range(1, 4):
    ...
    motif_usage_s3d = pd.read_csv(rf'{data_path}\S3D\motif_usage_overall.csv')
    ...
    for i in range(n_cluster):
        ...
        print(
            "          HAM_D-BD: rho: {:.2f}, p-val: {:.5f}".format(corr_HAM_D_score_BD[0][0], corr_HAM_D_score_BD[1]))
...
# %% Scored motif between motif paired t test and score correlation
for epoch in range(1, 4):
    motif_usage_score_ = np.asarray(eval("Epoch{}_motif_usage_score".format(epoch)))
    # motif_usage_score_[:, :, 0] = motif_usage_score_[:, :, 0] + motif_usage_score_[:, :, 1]
    # motif_usage_score_[:, :, 1] = motif_usage_score_[:, :, 2] + motif_usage_score_[:, :, 3]
    # motif_usage_score_[:, :, 2] = motif_usage_score_[:, :, 4] + motif_usage_score_[:, :, 5]
    # motif_usage_score_[:, :, 3] = motif_usage_score_[:, :, 6] + motif_usage_score_[:, :, 7]
    # motif_usage_score_[:, :, 4] = motif_usage_score_[:, :, 8]
    # motif_usage_score_[:, :, 5] = motif_usage_score_[:, :, 9]
    print("Epoch {}".format(epoch))
    motif_usage_cat_score = np.asarray(motif_usage_cat_score)
    for i, motif_behavior in enumerate(score_bahavior_names):
        ...
        if np.sum(HC_score) == 0 or np.sum(BD_score) == 0:
            print('HC not exist {}'.format(np.sum(HC_score) == 0))
            print("BD not exist{}".format(np.sum(BD_score) == 0))
...
for j in range(2):
    ...
    ds = pd.DataFrame(np.concatenate((
        np.concatenate((motif_usage_cat1[:, :].T.flatten(), motif_usage_cat2[:, :].T.flatten(),
                        motif_usage_cat3[:, :].T.flatten()), 0).reshape(-1, 1),
