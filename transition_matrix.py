...
for i in range(num_metrics):
    ...
    axes[i].set_title('{}'.format(metric_names[i]))
    axes[i].set_ylim([-1, 3])
...
for epoch in range(1,4):
    ...
    latent_ds = pd.DataFrame(np.concatenate((
        ...
        np.concatenate((num_zeros[0], num_zeros[1]), 0).reshape(-1, 1),
        np.concatenate((CP_idx, BD_idx), 0).reshape(-1, 1)), 1),
        columns=['entropy', 'distribution of #empty state', 'distribution of #p(state) = 1',
                 'distribution of #p(state) = 0', 'is_BD'])

    sns.violinplot(y="entropy", x='is_BD',
                   data=latent_ds, palette="muted", ax=axes[0])
    sns.stripplot(y="entropy", x="is_BD", data=latent_ds,
                  color="white", edgecolor="gray", ax=axes[0])
    axes[0].set_xticklabels(['CP', 'BD'])
    axes[0].set_xlabel('population')
    axes[0].set_title('distribution of entropy')
    axes[0].set_ylim([-1, 3])
    sns.violinplot(x='is_BD', y="distribution of #empty state",
                   data=latent_ds, palette="muted", ax=axes[1])
    sns.stripplot(y="distribution of #empty state", x="is_BD", data=latent_ds,
                  color="white", edgecolor="gray", ax=axes[1])
    axes[1].set_xticklabels(['CP', 'BD'])
    axes[1].set_title('distribution of #empty state')
    axes[1].set_ylim([-5, 15])
    axes[1].set_xlabel('population')
    sns.violinplot(x='is_BD', y="distribution of #p(state) = 1",
                   data=latent_ds, palette="muted", ax=axes[2])
    sns.stripplot(y="distribution of #p(state) = 1", x="is_BD", data=latent_ds,
                  color="white", edgecolor="gray", ax=axes[2])
    axes[2].set_xticklabels(['CP', 'BD'])
    axes[2].set_title('distribution of #p(state) = 1')
    axes[2].set_ylim([-2, 8])
    axes[2].set_xlabel('population')
    sns.violinplot(x='is_BD', y="distribution of #p(state) = 0",
                   data=latent_ds, palette="muted", ax=axes[3])
    sns.stripplot(y="distribution of #p(state) = 0", x="is_BD", data=latent_ds,
                  color="white", edgecolor="gray", ax=axes[3])
    axes[3].set_xticklabels(['CP', 'BD'])
    axes[3].set_title('distribution of #p(state) = 0')
    axes[3].set_ylim([50, 120])
