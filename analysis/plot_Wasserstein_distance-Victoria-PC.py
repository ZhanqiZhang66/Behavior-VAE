...
for i in range(n_subject_in_population):
    for j in range(3):
        for k in range(10):
            res[i][j][k] = distances_HC_HC_df[((distances_HC_HC_df['HCPerson1']==i) | (distances_HC_HC_df['HCPerson2']==i)) & (distances_HC_HC_df['Epoch']==(j+1)) & (distances_HC_HC_df['Motif']==k)]['Distance'].mean(skipna=True)
...
for i in range(n_subject_in_population):
    for j in range(3):
        for k in range(10):
            res_BD_BD[i][j][k] = distances_BD_BD_df[((distances_BD_BD_df['BDPerson1'] == i + n_subject_in_population) | (distances_BD_BD_df['BDPerson2'] == i+n_subject_in_population)) & (distances_BD_BD_df['Epoch']==(j+1)) & (distances_BD_BD_df['Motif']==k)]['Distance'].mean(skipna=True)
...
for i in range(n_subject_in_population):
    for ii in range(n_subject_in_population):
        for j in range(3):
            for k in range(10):
                res_HC_BD[i][j][k] = distances_HC_BD_df[((distances_HC_BD_df['HCPerson1'] == i) | (distances_HC_BD_df['BDPerson2'] == ii + n_subject_in_population)) & (distances_HC_BD_df['Epoch']==(j+1)) & (distances_HC_BD_df['Motif']==k)]['Distance'].mean(skipna=True)
